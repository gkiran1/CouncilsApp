...
export class AgendaPage {
    ...
    showList(event) {
        ...
        */
        ...
        let enteredSTN = '' + (<FormControl>this.newagendaForm.get('spiritualthought').value);

        if (assignedSTN === enteredSTN) {
            //do nothnig
        } else if ('@' + assignedSTN === enteredSTN) {
            (<FormControl>this.newagendaForm.controls['spiritualthought']).setValue(assignedSTN);
        } else {
            (<FormControl>this.newagendaForm.controls['spiritualthought']).setValue('');
        }

        let assignedCPN = this.closingprayer && (this.closingprayer.firstname + ' ' + this.closingprayer.lastname);
        let enteredCPN = '' + (<FormControl>this.newagendaForm.get('closingprayer').value);

        if (assignedCPN === enteredCPN) {
            //do nothnig
        } else if ('@' + assignedCPN === enteredCPN) {
            (<FormControl>this.newagendaForm.controls['closingprayer']).setValue(assignedCPN);
        } else {
            (<FormControl>this.newagendaForm.controls['closingprayer']).setValue('');
        }
    }
    ...
    showList1(event) {
        ...
        */
        ...
        let enteredOPN = '' + (<FormControl>this.newagendaForm.get('openingprayer').value);

        if (assignedOPN === enteredOPN) {
            //do nothnig
        } else if ('@' + assignedOPN === enteredOPN) {
            (<FormControl>this.newagendaForm.controls['openingprayer']).setValue(assignedOPN);
        } else {
            (<FormControl>this.newagendaForm.controls['openingprayer']).setValue('');
        }
